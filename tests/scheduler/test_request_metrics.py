...
def test_request_metrics_count_caller_supplied_generated_prefix_as_input() -> None:
    request = GenerationRequest(
        request_id=1,
        prompt="prompt",
        prompt_tokens=[TextToken(1), TextToken(2)],
...
def test_request_metrics_exclude_internal_preemption_generated_prefix() -> None:
    request = GenerationRequest(
        request_id=1,
        prompt="prompt",
        prompt_tokens=[TextToken(1), TextToken(2)],
    ...
    lifecycle = RequestLifecycle(request=request, skill_state=object())
    request.lifecycle = lifecycle
    request.generated_prefix = GeneratedPrefix(
        tokens=(TextToken(10), TextToken(11), TextToken(12)),
    )
