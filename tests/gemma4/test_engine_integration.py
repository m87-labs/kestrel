"""End-to-end query through Gemma4Runtime."""
...
def test_engine_query_returns_text() -> None:
    ...
    async def run() -> str:
        ...
        try:
            result = await engine.model(_MODEL_ID).query(
                image=None,
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="requires CUDA")
def test_engine_batches_distinct_gemma4_queries() -> None:
    import asyncio
