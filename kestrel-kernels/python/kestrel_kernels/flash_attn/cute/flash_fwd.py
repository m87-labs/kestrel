...
from kestrel_kernels.flash_attn.cute import ampere_helpers as sm80_utils
...
from kestrel_kernels.flash_attn.cute.mask import AttentionMask
from kestrel_kernels.flash_attn.cute.softmax import Softmax, apply_score_mod_inner
from kestrel_kernels.flash_attn.cute.seqlen_info import SeqlenInfoQK
from kestrel_kernels.flash_attn.cute.block_info import BlockInfo
from kestrel_kernels.flash_attn.cute.block_sparsity import BlockSparseTensors
from kestrel_kernels.flash_attn.cute.block_sparse_utils import (
...
from kestrel_kernels.flash_attn.cute.tile_scheduler import (
...
class FlashAttentionForwardSm90(FlashAttentionForwardBase):
    ...
    def __call__(
        ...
        use_cp_async_paged_kv = mPageTable is not None and self.paged_kv_non_tma
        # TMA K/V loads only require 1 warp. Non-TMA (cp.async) uses the full producer warp-group.
        self.num_producer_threads = 128 if use_cp_async_paged_kv else 32
        ...
        self.use_tma_Q = (
            not use_cp_async_paged_kv
            and self.arch >= 90
            and not (self.pack_gqa and self.tile_m % self.qhead_per_kvhead != 0)
        )
        self.use_tma_KV = not use_cp_async_paged_kv
        self.use_tma_O = (
        ...
        tma_atom_K, tma_tensor_K = None, mK
        tma_atom_V, tma_tensor_V = None, mV
    ...
    def kernel(
        ...
        pipeline_kv_producer_group = cutlass.pipeline.CooperativeGroup(
            ...
            self.num_producer_threads if const_expr(not self.use_tma_KV) else 1,
        )
        ...
        if const_expr(self.use_tma_KV):
            ...
            pipeline_k = pipeline.PipelineCpAsync.create(
                barrier_storage=storage.mbar_ptr_K.data_ptr(),
                num_stages=self.num_stages,
                producer_group=pipeline_kv_producer_group,
                consumer_group=pipeline_kv_consumer_group,
            ...
            pipeline_v = pipeline.PipelineCpAsync.create(
                barrier_storage=storage.mbar_ptr_V.data_ptr(),
                num_stages=self.num_stages,
                producer_group=pipeline_kv_producer_group,
                consumer_group=pipeline_kv_consumer_group,
    ...
    def load(
        ...
        use_cp_async_paged_kv = mPageTable is not None and self.paged_kv_non_tma
        # Predeclare to keep types stable across dynamic control flow.
        kv_producer_state = pipeline.make_pipeline_state(
            cutlass.pipeline.PipelineUserType.Producer, self.num_stages
        )
        tile_scheduler = TileSchedulerCls()
        work_tile = tile_scheduler.initial_work_tile_info()
        if const_expr(use_cp_async_paged_kv):
            # Paged KV non-TMA path: use cp.async loads (PipelineCpAsync) instead of TMA.
            # Use all producer warps (128 threads) for K/V loads.
            tidx, _, _ = cute.arch.thread_idx()
            # Setup page table divmod once outside the tile loop.
            page_size = mK.shape[0]
            page_size_divmod = FastDivmodDivisor(page_size)

            # Use the same 128-bit cp.async atom as the regular KV loads.
            universal_copy_bits = 128
            async_copy_elems = universal_copy_bits // mK.element_type.width
            atom_async_copy = cute.make_copy_atom(
                cpasync.CopyG2SOp(cache_mode=cpasync.LoadCacheMode.GLOBAL),
                mK.element_type,
                num_bits_per_copy=universal_copy_bits,
            ...
            stage_idx = kv_producer_state.index if const_expr(self.num_stages > 1) else Int32(0)
            m_block0 = Int32(0)
            while work_tile.is_valid_tile:
                ...
                if const_expr(self.use_block_sparsity):
                    # Not yet implemented for this path.
                    assert False

                # Slice by head once; per-row selection uses pointer arithmetic (non-affine
                # page_idx -> pointer), which keeps cp.async sources as memrefs (not coord_tensors).
                mK_head = mK[None, None, head_idx_kv, None]
                mV_head = mV[None, None, head_idx_kv, None]
                ...
                if const_expr(self.intra_wg_overlap):
                    # Match the KV overlap schedule used by the TMA path: keep K one stage ahead of V.
                    tPrVPtr0 = cute.make_rmem_tensor((cute.size(tKsK, mode=[1]),), cutlass.Int64)
                    tPrVPtr1 = cute.make_rmem_tensor((cute.size(tKsK, mode=[1]),), cutlass.Int64)
                    tPrVPtr_prev = tPrVPtr0
                    tPrVPtr_next = tPrVPtr1

                    # First iteration: compute ptrs, stash V ptrs, and load only K for the first block.
                    n_block = n_block_max - 1
                    seqlenk_row_limit = (
                        seqlen.seqlen_k - n_block * self.tile_n if n_block >= 0 else 0
                    )
                    ...
                    for m in cutlass.range_constexpr(cute.size(tKsK, mode=[1])):
                        ...
                        if should_load_row:
                            ...
                            for k in cutlass.range(cute.size(tKsK, mode=[2]), unroll=1):
                                ...
                                cute.copy(
                                    atom_async_copy,
                                    mK_paged_row_copy[None, ki],
                                    tKsK[None, m, k, stage_idx],
                                )
                    pipeline_k.producer_commit(kv_producer_state)

                    # Remaining iterations: overlap K(next block) with V(prev block).
                    for i in cutlass.range(n_block_max - 1 - n_block_min, unroll=1):
                        kv_producer_state_prev = kv_producer_state.clone()
                        stage_idx_prev = stage_idx
                        ...
                        for m in cutlass.range_constexpr(cute.size(tVsV, mode=[1])):
                            v_ptr_i64 = tPrVPtr_prev[m]
                            should_load_row = v_ptr_i64 != cutlass.Int64(0)
                        ...
                        pipeline_v.producer_commit(kv_producer_state_prev)

                        # Rotate the per-block V pointer buffers.
                        tPrVPtr_tmp = tPrVPtr_prev
                        tPrVPtr_prev = tPrVPtr_next
                        tPrVPtr_next = tPrVPtr_tmp

                    # Final step: load V for the last block and advance the producer state.
                    ...
                    kv_producer_state.advance()
                    stage_idx = (
                        kv_producer_state.index if const_expr(self.num_stages > 1) else stage_idx
                    )
