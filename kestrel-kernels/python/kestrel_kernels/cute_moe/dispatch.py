...
_COMPILE_CACHE_W8A16: Dict[Tuple[str, CuteMoeConfig], Any] = {}
...
def _invoke_cute_moe_w8a16_impl(
    kind: str,
    A: torch.Tensor,  # BF16 activations (no quantization needed)
    B_fp8_bits: torch.Tensor,
    B_scale: torch.Tensor,
    C: torch.Tensor,
    *,
    ...
    """W8A16 dispatch: FP8 weights with BF16 activations (warp kernel only).

    This avoids the activation quantization overhead of W8A8 while still using
    FP8 weights. Only valid for warp kernel (kernel_type="warp").
    """
    if A.dtype != torch.bfloat16:
        raise ValueError(f"W8A16 expects BF16 activations (got {A.dtype})")
    if B_fp8_bits.dtype != torch.uint8:
        raise ValueError(f"Expected FP8 weight bits as uint8 (got {B_fp8_bits.dtype})")
    if B_scale.dtype not in (torch.float16, torch.float32):
        raise ValueError(f"Expected B_scale float16/float32 (got {B_scale.dtype})")
    if C.dtype != torch.bfloat16:
        raise ValueError(f"W8A16 expects bfloat16 C (got {C.dtype})")
    ...
    if C.ndim != 3:
        raise ValueError("C must be 3D [M, top_k, N]")
    if A.stride(-1) != 1:
        raise ValueError("A must be contiguous in the last dimension (K)")
    if B_fp8_bits.stride(-1) != 1:
        raise ValueError("B_fp8_bits must be contiguous in the last dimension (K)")
    if C.stride(-1) != 1:
        raise ValueError("C must be contiguous in the last dimension (N)")
    ...
    if num_tokens_post_padded.dtype != torch.int32:
        raise ValueError("num_tokens_post_padded must be int32")
    if config.kernel_type != "warp":
        raise ValueError("W8A16 only supports warp kernel (kernel_type='warp')")
    if mul_routed_weight:
        if topk_weights is None:
            raise ValueError("topk_weights is required when mul_routed_weight=True")
    else:
    ...
    # Ensure B_scale is float32
    if B_scale.dtype != torch.float32:
        B_scale = B_scale.to(dtype=torch.float32)

    if key not in _COMPILE_CACHE_W8A16:
        if _ENABLE_JIT:
            # JIT compile
            a_cute = _to_cute_tensor_2d_contig(A)
            b_bits_cute = _to_cute_tensor_3d_last_contig_u8(B_fp8_bits)
            ...
            op = _FusedMoeMatmulCuTeWarpW8A16(
                dtype, fp8_dtype, config, mul_routed_weight=mul_routed_weight, top_k=top_k,
                N=N_dim, K=K_dim,
            )
            compiled = cute.compile(
            ...
            _set_compiled_kernel_shared_carveout(compiled)
            _COMPILE_CACHE_W8A16[key] = compiled
        else:
            # W8A16 requires JIT until kernel correctness is fixed
            # TODO: Add precompiled W8A16 kernels once the kernel is validated
            raise RuntimeError(
                f"W8A16 kernel requires JIT compilation. Set KESTREL_CUTE_MOE_JIT=1 "
                f"or use W8A8 (FP8 activations) for precompiled kernels."
            )

    _COMPILE_CACHE_W8A16[key](
        A,
        B_fp8_bits,
        B_scale,
        C2d,
...
def invoke_cute_moe_up_w8a16(
    A: torch.Tensor,  # BF16 activations
    B_fp8_bits: torch.Tensor,
    B_scale: torch.Tensor,
    C: torch.Tensor,
    *,
    ...
    """CuTe fused MoE up-projection with W8A16 (FP8 weights, BF16 activations).

    This is faster than W8A8 for small batches because it skips activation
    quantization entirely. Only available for warp kernel (kernel_type='warp').
...
def invoke_cute_moe_down_w8a16(
    A: torch.Tensor,  # BF16 activations
    B_fp8_bits: torch.Tensor,
    B_scale: torch.Tensor,
    C: torch.Tensor,
    *,
    ...
    """CuTe fused MoE down-projection with W8A16 (FP8 weights, BF16 activations).
