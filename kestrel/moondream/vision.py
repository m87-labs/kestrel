...
def vision_encoder(
    crops: torch.Tensor, module: nn.Module, config: VisionConfig
) -> torch.Tensor:
    ...
    x = F.layer_norm(
        x, module.post_ln.normalized_shape, module.post_ln.weight, module.post_ln.bias
    )
    return x


def vision_encoder_multiscale(
    crops: torch.Tensor, module: nn.Module, config: VisionConfig, early_block: int = 8
