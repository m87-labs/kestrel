...
class ChatSkillState(SkillState):
    ...
    @property
    def mask_is_stateful(self) -> bool:
        # A reasoning run can cross into a forced prefix even when its current
        # mask is empty. Answer-only runs and empty prefixes cannot do so.
        pending = (self._post_reasoning_tokens is not None
                   and self._post_reasoning_idx < len(self._post_reasoning_tokens))
        return pending or (self._collecting_reasoning and bool(self._post_reasoning_prefix))
