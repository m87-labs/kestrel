...
class MaskEncoder(nn.Module):
    def __init__(self, embed_dim):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(
                embed_dim // 16, embed_dim // 8, kernel_size=3, stride=2, padding=1
            ),
        ...
        self.conv3 = nn.Sequential(
            nn.Conv2d(
                embed_dim // 8, embed_dim // 4, kernel_size=3, stride=2, padding=1
            ),
...
class TwoWayBlock(nn.Module):
    def __init__(self, embed_dim, num_heads=8, mlp_ratio=8):
        ...
        self.output_self_attn = nn.MultiheadAttention(
...
class HQSAMHead(nn.Module):
    def __init__(
        self, enc_dim=1152, embed_dim=256, num_heads=8, num_layers=2, num_masks=4
    ):
        super().__init__()
        ...
        self.final_token_to_image = nn.MultiheadAttention(
            embed_dim, num_heads, batch_first=True
        )
        ...
        self.final_attn_up = nn.MultiheadAttention(
            embed_dim // 8, num_heads=4, batch_first=True
        )
    ...
    def forward(self, final_features, early_features, coarse_mask):
        ...
        for block in self.transformer_blocks:
            output_tokens, img_tokens = block(
                output_tokens, img_tokens, output_pe, img_pe
            )
...
class HQSAMHeadRefiner:
    ...
    def __call__(self, final_features, early_features, mask, n_iters=5):
        ...
        for _ in range(n_iters):
            all_logits, iou_pred = self.head(
                final_features, early_features, current_mask
            )
