...
class _FusedMoeMatmulCuTeFp8W:
    """Decode-specialized routed MoE GEMM with FP8 weights (stored as raw uint8 bits).

    Math:
      C = (A @ (B_fp8)^T) * B_scale   (and optionally * routed_weight)

    - A is BF16.
    - B_fp8 is float8_e4m3fn stored as raw bytes (uint8 view), representing (W / scale).
    - B_scale is per-(expert, out_channel) fp32 scale.
    """
    ...
    def _shared_storage_cls(self):
        ...
        sC_elems = self.config.block_m * self.config.block_n
        sMeta_elems = self.config.block_m
        sBScale_elems = self.config.block_n
        sA_struct = cute.struct.Align[cute.struct.MemRange[self.dtype, sA_elems], 1024]
        sB_struct = cute.struct.Align[cute.struct.MemRange[self.dtype, sB_elems], 1024]
        sC_struct = cute.struct.Align[cute.struct.MemRange[self.dtype, sC_elems], 16]
        sAid_struct = cute.struct.Align[cute.struct.MemRange[Int32, sMeta_elems], 16]
    ...
    def __call__(
        self,
        mA: cute.Tensor,  # (M_tokens_or_assignments, K) BF16
    ...
    def kernel(
        ...
        do_compute = block_active and (expert_id != -1)

        # Load per-output-channel scales for this expert + N tile.
        for n_local in range(tx, block_n, num_threads):
            col = n_start + Int32(n_local)
            scale = Float32(0.0)
            if do_compute and (col < N):
                scale = Float32(mBScale[expert_id, col])
            sBScale[n_local] = scale
        cute.arch.barrier()

        # Copy atoms for A (cp.async gather) and B (fp8->bf16 conversion).
        vec_size = 8
        copy_bits = vec_size * self.dtype.width
        atom_async_copy_a = cute.make_copy_atom(
            cpasync.CopyG2SOp(cache_mode=cpasync.LoadCacheMode.GLOBAL),
        ...
        atom_store_b = cute.make_copy_atom(
            cute.nvgpu.CopyUniversalOp(),
            self.dtype,
            num_bits_per_copy=copy_bits,
        )
        ...
        for stage_prefetch in cutlass.range_constexpr(num_stages):
            ...
            for vec_linear_a in range(tx, total_vec_a, num_threads):
                ...
                valid_a = tile_in_range and valid_row_a and (kg_a < K) and do_compute
                g_off_bytes_a = arow_base + cutlass.Int64(kg_a) * element_bytes
                g_ptr_a = cute.make_ptr(
                    self.dtype,
                    mA_base_i64 + g_off_bytes_a,
            ...
            # B tile: fp8 -> bf16 conversion into swizzled SMEM.
            stage_off_bytes_b = (
                cutlass.Int64(Int32(stage_prefetch) * stage_stride_b) * element_bytes
            )
            sB_stage_ptr = cute.make_ptr(
                self.dtype,
                sB_base_i64 + stage_off_bytes_b,
            ...
            full_k_tile_prefetch = (k_start_prefetch + Int32(block_k)) <= K
            for vec_linear_b in range(tx, total_vec_b, num_threads):
                n_b = vec_linear_b // block_vec_k
                kvec_b = vec_linear_b - n_b * block_vec_k
                k_b = Int32(kvec_b * vec_size)
                ng_b = n_start + Int32(n_b)
                kg_b = k_start_prefetch + k_b

                rB = cute.make_rmem_tensor((vec_size,), self.dtype)
                rB.fill(self.dtype(0.0))

                if tile_in_range and (ng_b < N) and (kg_b < K) and do_compute:
                    base_off_u8 = (
                    ...
                    if full_k_tile_prefetch:
                        src_b = cute.make_tensor(g_ptr_b, (vec_size,))
                        rB.store(src_b.load().to(Float32).to(self.dtype))

                s_ptr_b = cute.make_ptr(
                    self.dtype,
                    sB_stage.iterator.toint()
                    + cutlass.Int64(Int32(sB_tile_layout((Int32(n_b), k_b)))) * element_bytes,
                    cute.AddressSpace.smem,
                    assumed_align=align_bytes,
                )
                dst_b = cute.make_tensor(s_ptr_b, (vec_size,))
                cute.copy(atom_store_b, rB, dst_b)
        ...
        for tile_idx in cutlass.range(main_tiles, unroll=1):
            ...
            if next_tile < k_tiles:
                ...
                for vec_linear_a2 in range(tx, total_vec_a, num_threads):
                    ...
                    valid_a2 = valid_row_a2 and (kg_a2 < K) and do_compute
                    g_off_bytes_a2 = arow_base2 + cutlass.Int64(kg_a2) * element_bytes
                    g_ptr_a2 = cute.make_ptr(
                        self.dtype,
                        mA_base_i64 + g_off_bytes_a2,
                ...
                # Prefetch next B tile (fp8 -> bf16) into this stage.
                stage_off_bytes_b2 = cutlass.Int64(stage_idx * stage_stride_b) * element_bytes
                sB_stage_ptr2 = cute.make_ptr(
                    self.dtype,
                    sB_base_i64 + stage_off_bytes_b2,
                    cute.AddressSpace.smem,
                ...
                for vec_linear_b2 in range(tx, total_vec_b, num_threads):
                    ...
                    k_b2 = Int32(kvec_b2 * vec_size)
                    ng_b2 = n_start + Int32(n_b2)
                    kg_b2 = k_start_next + k_b2

                    rB2 = cute.make_rmem_tensor((vec_size,), self.dtype)
                    rB2.fill(self.dtype(0.0))

                    if (ng_b2 < N) and (kg_b2 < K) and do_compute:
                        base_off_u8_2 = (
                            cutlass.Int64(expert_id) * stride_be_u8
                        ...
                        if full_k_tile_next:
                            src_b2 = cute.make_tensor(g_ptr_b2, (vec_size,))
                            rB2.store(src_b2.load().to(Float32).to(self.dtype))

                    s_ptr_b2 = cute.make_ptr(
                        self.dtype,
                        sB_stage2.iterator.toint()
                        + cutlass.Int64(Int32(sB_tile_layout((Int32(n_b2), k_b2))))
                        * element_bytes,
                        cute.AddressSpace.smem,
                        assumed_align=align_bytes,
                    )
                    dst_b2 = cute.make_tensor(s_ptr_b2, (vec_size,))
                    cute.copy(atom_store_b, rB2, dst_b2)
        ...
        # Epilogue: apply per-channel fp32 scale (and routed weight), then store/scatter.
        rC = cute.make_fragment_like(acc, self.dtype)
        cC = cute.make_identity_tensor((block_m, block_n))
        tC_coords = fa_utils.make_acc_tensor_mn_view(thr_mma.partition_C(cC))
        tAcc = fa_utils.make_acc_tensor_mn_view(acc)
        ...
        for mi in cutlass.range_constexpr(cute.size(tAcc.shape[0])):
            ...
            for ni in cutlass.range_constexpr(cute.size(tAcc.shape[1])):
                ...
                scale = sBScale[n]
                tRC[mi, ni] = self.dtype(Float32(tAcc[mi, ni]) * scale * w32)
...
_COMPILE_CACHE_FP8W: Dict[Tuple[str, FusedMoeCuTeConfig], Any] = {}
...
def _invoke_fused_moe_kernel_cute_fp8w_impl(
    kind: str,
    A: torch.Tensor,
    ...
    *,
    ...
    if A.dtype != torch.bfloat16:
        raise ValueError(f"CuTe fused MoE FP8W supports bfloat16 A only (got {A.dtype})")
    ...
    if not (A.is_cuda and B_fp8_bits.is_cuda and B_scale.is_cuda and C.is_cuda):
        raise ValueError("A/B_fp8_bits/B_scale/C must be CUDA tensors")
    if A.ndim != 2:
        raise ValueError("A must be 2D")
    ...
    if C.stride(-1) != 1:
        raise ValueError("C must be contiguous in the last dimension (N)")
    if B_fp8_bits.shape[2] != A.shape[1]:
        raise ValueError("A and B_fp8_bits must have the same K dimension")
    if int(A.shape[1]) % int(config.block_k) != 0:
        raise ValueError("CuTe FP8W kernel requires K divisible by block_k")
    ...
    stream = cuda.CUstream(torch.cuda.current_stream().cuda_stream)
    if key not in _COMPILE_CACHE_FP8W:
        op = _FusedMoeMatmulCuTeFp8W(
        ...
        _set_compiled_kernel_shared_carveout(compiled)
        _COMPILE_CACHE_FP8W[key] = compiled

    _COMPILE_CACHE_FP8W[key](
        a_cute,
        b_bits_cute,
        b_scale_cute,
        c_cute,
...
def invoke_fused_moe_kernel_cute_up_decode_fp8w(
    A: torch.Tensor,
    B_fp8_bits: torch.Tensor,
    B_scale: torch.Tensor,
    C: torch.Tensor,
    *,
    ...
    """Decode-specialized CuTe fused MoE up-projection with FP8 weights."""
    _invoke_fused_moe_kernel_cute_fp8w_impl(
        "up",
        A,
        B_fp8_bits,
        B_scale,
...
def invoke_fused_moe_kernel_cute_down_decode_fp8w(
    A: torch.Tensor,
    B_fp8_bits: torch.Tensor,
    B_scale: torch.Tensor,
    C: torch.Tensor,
    *,
    ...
    """Decode-specialized CuTe fused MoE down-projection with FP8 weights."""
    _invoke_fused_moe_kernel_cute_fp8w_impl(
        "down",
        A,
        B_fp8_bits,
