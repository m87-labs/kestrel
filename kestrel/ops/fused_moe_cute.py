...
_TORCH_TO_CUTLASS: dict[torch.dtype, type[cutlass.Numeric]] = {
...
class _FusedMoeMatmulCuTeDecodeWarp:
    """Decode-specialized kernel variant that avoids CTA-wide barriers.

    Strategy:
      - Replicate A tiles per-warp (each warp reads only what it loads).
      - Partition B/C tiles by warp along N (each warp owns a contiguous N slice).
      - Use `cp_async_wait_group` + `sync_warp` instead of `barrier`.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if config.block_n % config.num_warps != 0:
            raise ValueError("Warp decode kernel requires block_n divisible by num_warps")
        block_n_warp = config.block_n // config.num_warps
        # This kernel assumes the MMA's internal N permutation splits the warp's N slice
        # into two contiguous segments (one in each half of the tile).
        if block_n_warp % 2 != 0:
            raise ValueError("Warp decode kernel requires (block_n/num_warps) divisible by 2")
        block_n_seg = block_n_warp // 2
        # We use vec_size=8 (128-bit) loads/stores; require segment N to be a multiple.
        if block_n_seg % 8 != 0:
            raise ValueError("Warp decode kernel requires (block_n/(2*num_warps)) divisible by 8")
        self.dtype = dtype
        self.config = config
        self.mul_routed_weight = mul_routed_weight
        self.top_k = int(top_k)
    ...
    def _shared_storage_cls(self):
        # Replicate A per-warp to avoid inter-warp dependencies.
        sA_elems = (
    ...
    def kernel(
        ...
        warp_id = tx // Int32(32)
        lane_id = tx - warp_id * Int32(32)

        block_n_warp = block_n // num_warps
        # For this MMA tile, the N dimension is permuted into two halves.
        # Each warp owns a contiguous segment in each half.
        block_n_seg = block_n_warp // 2
        half_n = block_n // 2
        warp_n_seg0 = warp_id * Int32(block_n_seg)
        warp_n_seg1 = Int32(half_n) + warp_n_seg0
        ...
        sA_tile_layout = cute.tile_to_shape(sB_layout_atom, (block_m, block_k), (0, 1))
        sB_tile_layout = cute.tile_to_shape(sB_layout_atom, (block_n, block_k), (0, 1))
        sA_elems = num_warps * num_stages * block_m * block_k
        sB_elems = num_stages * block_n * block_k
        sA = storage.sA.get_tensor(cute.make_layout((sA_elems,), stride=(1,)))
        sB = storage.sB.get_tensor(cute.make_layout((sB_elems,), stride=(1,)))
        # Flash-attn-style swizzled SMEM layout for C. This makes `stmatrix` stores conflict-free
        ...
        if (not block_active) or (expert_id == -1):
            ...
            block_vec_k = block_k // vec_size
            total_vec_a = block_m * block_vec_k
            stage_stride_a = block_m * block_k
            warp_stride_a = num_stages * stage_stride_a
            stage_stride_b = block_n * block_k
            k_tiles = cute.ceil_div(K, block_k)
            full_n_tile = (n_start + Int32(block_n)) <= N
            ...
            for stage_prefetch in cutlass.range_constexpr(num_stages):
                ...
                # A tile (replicated per warp): [block_m, block_k]
                for vec_linear_a in range(lane_id, total_vec_a, 32):
                    r_a = vec_linear_a // block_vec_k
                    kvec_a = vec_linear_a - r_a * block_vec_k
                    k_a = Int32(kvec_a * vec_size)
                    kg_a = k_start_prefetch + k_a
                    ...
                    s_linear_a = (
                        warp_id * warp_stride_a
                        + Int32(stage_prefetch) * stage_stride_a
                        + Int32(sA_tile_layout((Int32(r_a), k_a)))
                    )
                    s_off_bytes_a = cutlass.Int64(s_linear_a) * element_bytes
                    s_ptr_a = cute.make_ptr(
                        self.dtype,
                ...
                if tile_in_range:
                    ...
                    stage_off_elems_b = Int32(stage_prefetch) * Int32(stage_stride_b)
                    stage_off_bytes_b = cutlass.Int64(stage_off_elems_b) * element_bytes
                    sB_stage_ptr = cute.make_ptr(
                        self.dtype,
                        sB_base_i64 + stage_off_bytes_b,
            ...
            for tile_idx in cutlass.range(main_tiles, unroll=1):
                ...
                stage_off_elems_a = warp_id * Int32(warp_stride_a) + stage_idx * Int32(stage_stride_a)
                stage_off_elems_b = stage_idx * Int32(stage_stride_b)
                stage_off_bytes_a = cutlass.Int64(stage_off_elems_a) * element_bytes
                ...
                # B is shared across warps; ensure all warps are done reading this stage
                # before overwriting it with the next tile.
                cute.arch.barrier()
                ...
                if next_tile < k_tiles:
                    ...
                    for vec_linear_a2 in range(lane_id, total_vec_a, 32):
                        r_a2 = vec_linear_a2 // block_vec_k
                        kvec_a2 = vec_linear_a2 - r_a2 * block_vec_k
                        k_a2 = Int32(kvec_a2 * vec_size)
                        kg_a2 = k_start_next + k_a2
                        ...
                        s_linear_a2 = (
                            warp_id * warp_stride_a
                            + stage_idx * stage_stride_a
                            + Int32(sA_tile_layout((Int32(r_a2), k_a2)))
                        )
                        s_off_bytes_a2 = cutlass.Int64(s_linear_a2) * element_bytes
                        s_ptr_a2 = cute.make_ptr(
                            self.dtype,
                    ...
                    stage_off_elems_b2 = stage_idx * Int32(stage_stride_b)
                    stage_off_bytes_b2 = cutlass.Int64(stage_off_elems_b2) * element_bytes
                    sB_stage_ptr2 = cute.make_ptr(
                        self.dtype,
                        sB_base_i64 + stage_off_bytes_b2,
            ...
            vec_n_seg = block_n_seg // vec_size
            total_vec_c_seg = block_m * vec_n_seg

            for n_seg in cutlass.range_constexpr(2):
                n_base_c = warp_n_seg0 if const_expr(n_seg == 0) else warp_n_seg1
                for vec_linear_c in range(lane_id, total_vec_c_seg, 32):
                    r_c = vec_linear_c // vec_n_seg
                    nvec_c = vec_linear_c - r_c * vec_n_seg
                    n0 = Int32(nvec_c * vec_size)
                    aid_c = sAid[r_c]
...
_COMPILE_CACHE: Dict[
    Tuple[str, type[cutlass.Numeric], FusedMoeCuTeConfig, bool, int], Any
] = {}
...
def invoke_fused_moe_kernel_cute(
    ...
    *,
    ...
    """CuTe DSL equivalent of `kestrel.fused_moe.kernels.invoke_fused_moe_kernel`.

    Notes:
      - Specializes for BF16/FP16 on SM90.
      - Bias and TF32 paths are intentionally omitted (not used in Moondream).
    """
    if A.dtype not in _TORCH_TO_CUTLASS:
        raise ValueError(f"Unsupported dtype for CuTe fused MoE: {A.dtype}")
    ...
    dtype = _TORCH_TO_CUTLASS[A.dtype]
    variant_env = os.getenv("KESTREL_CUTE_FUSED_MOE_VARIANT", "").strip().lower()
    if variant_env:
        if variant_env in ("cta", "default", "baseline"):
            variant = "cta"
        elif variant_env in ("warp", "decode-warp", "decode_warp", "warp-decode", "warp_decode"):
            variant = "warp"
        else:
            raise ValueError(
                "Unknown KESTREL_CUTE_FUSED_MOE_VARIANT (expected 'cta' or 'warp')"
            )
    else:
        # Default to the CTA kernel. It uses much less shared memory than the warp-replicated
        # variant and typically achieves better decode throughput on H100.
        variant = "cta"

    key = (variant, dtype, config, bool(mul_routed_weight), int(top_k))
    ...
    if key not in _COMPILE_CACHE:
        if variant == "warp":
            op = _FusedMoeMatmulCuTeDecodeWarp(
                dtype, config, mul_routed_weight=mul_routed_weight, top_k=top_k
            )
        else:
...
__all__ = ["FusedMoeCuTeConfig", "invoke_fused_moe_kernel_cute"]
